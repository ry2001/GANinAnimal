...
class GANinAnimalModel(BaseModel):
    """
    ...
    def set_input(self, input):
        ...
        self.image_paths = input['A_paths' if AtoB else 'B_paths'][0]
        self.labels_A = input['A_paths'][1]
        self.labels_B = input['B_paths'][1]
...
#         self.image_paths = input['A_paths' if AtoB else 'B_paths']
    ...
    def forward(self):
        ...
        self.fake_B_label = self.labels_A
        self.rec_A = self.netG_B(self.fake_B)   # G_B(G_A(A))
        self.fake_A = self.netG_B(self.real_B)  # G_B(B)
        self.fake_A_label = self.labels_B
...
#         pred_fake = netD(fake.detach())
    ...
    def backward_D_A(self):
        ...
        fake_B = self.fake_B_pool.query_cond(
            self.fake_B, self.fake_B_label, self.labels_B)
...
#         fake_B = self.fake_B_pool.query(self.fake_B)
    ...
    def backward_D_B(self):
        ...
        fake_A = self.fake_A_pool.query_cond(
            self.fake_A, self.fake_A_label, self.labels_A)
...
#         fake_A = self.fake_A_pool.query(self.fake_A)
    ...
    def backward_G(self):
        ...
        # FID score loss
        if self.fid:
            self.loss_fid_A = EvalGAN(
                self.inception_model, self.real_A, self.fake_A).calculate_fid()
            self.loss_fid_B = EvalGAN(
                self.inception_model, self.real_B, self.fake_B).calculate_fid()
            # combined loss and calculate gradients (added FID score here also)
            self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + \
                self.loss_idt_A + self.loss_idt_B + self.loss_fid_A + self.loss_fid_B
